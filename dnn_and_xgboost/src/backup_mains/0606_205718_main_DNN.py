...
for bs in batch_sizes:
    ...
    # 0 hyperparameters
    optimizer = rmsprop(lr=.0001) # rmsprop, adam 'adagrad'
    ...
    if args.et or args.ct:
        ...
        if args.m=='mlp_fe': # multilayer perceptrons
            ...
            for i,fe in enumerate(max_f_cols):
                ...
                if args.ne: col_out = Embedding(int(fe), 16, name=features[i])(col_in)
                else: col_out = Embedding(int(fe), 16, name=features[i])(col_in)
            ...
            if not args.ne:
                ...
                print('\nLoaded sythesized matrices to embedding layers')
...
####Namespace(b=4096, ct=False, e=5, et=None, f=16, fra=False, m='mlp_fe', mess='compare fixed emb_dims with variable ones', mt=3, ne=True, nfe=False, nm=False, ns=False, of=False, olv=False, rml=False, s=False, tdo=False, v=1, va=True, va_seed=62)