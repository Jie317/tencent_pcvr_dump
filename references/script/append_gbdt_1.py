...
def load_name_sample(input,gbdt,isTest):
    ...
    while True:
        ...
        if isApp == False:
            if isTest == True:
                print(str(label)+" "+" ".join(cur_str_x), file=fm_test_1)
            else:
                print(str(label)+" "+" ".join(cur_str_x), file=fm_train_1)
        else:
            if isTest == True:
                print(str(label)+" "+" ".join(cur_str_x), file=fm_test_2)             
            else:
                print(str(label)+" "+" ".join(cur_str_x), file=fm_train_2)
        cnt = cnt + 1
        if cnt % 100000 == 0:        
            print(cnt)
    f.close()
