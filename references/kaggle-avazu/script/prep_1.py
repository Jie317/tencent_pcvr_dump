...
ftr = "../train_c"
fte = "../test_c"
fset = marshal.load(open("../fc", 'rb'))
rare_d = marshal.load(open("../rare_d", 'rb'))
ftrain = "../train_pre_1"
ftest = "../test_pre_1"

id_day = marshal.load(open("../id_day", 'rb'))

def prep(input,output,isTest):
    f = open(input)
    out = open(output,"w")
    line = f.readline()
    print(line[:-1]  + ",C29", file=out)
    count = 0
    bias = 3
    if isTest:
        bias = 2
